...
/**
 * Sticky footer with the clear / apply buttons. Its own component so the
 * `useSafeAreaInsets` call resolves against the nested `SafeAreaProvider` inside
 * the `Modal` (RN renders modals in a separate native window, so the root
 * provider's insets don't apply) — otherwise, with Android 3-button navigation,
 * the system bar sits on top of the buttons.
 */
function SheetActions({ onClear, onApply }: SheetActionsProps) {
  ...
  return (
    <XStack
      ...
      px={20}
      pt={14}
      pb={Math.max(insets.bottom, 12) + 14}
      bg="$ji-white"
      borderTopWidth={1}
                ...
                <XStack flexWrap="wrap" gap={7}>
                  {SALARY_STEPS.map((step) => (
                    <ChoiceChip
                      key={step}
                      variant="pill"
                      label={salaryStepLabel(step)}
                      active={draft.salaryMin === step}
                      // Tap the active chip again to clear the salary filter.
                      onPress={() =>
                        setDraft((prev) => ({
                          ...prev,
                          salaryMin: prev.salaryMin === step ? 0 : step,
                        }))
                      }
                    />
                  ))}
